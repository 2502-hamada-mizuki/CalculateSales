...
public class CalculateSales {
	...
	private static final String BRANCH_LST = "支店定義";
	...
	private static final String COMMODITY_LST = "商品定義";
	...
	public static void main(String[] args) {
		...
		if(!readFile(args[0], FILE_NAME_BRANCH_LST, BRANCH_LST, branchNames, branchSales, branchCode)) {
			//falseだったらreturnで処理を終了させる（次へ進む）
			return;
		}

		// 商品定義ファイル読み込み処理
		if(!readFile(args[0], FILE_NAME_COMMODITY_LST, COMMODITY_LST, commodityNames, commoditySales, commodityCode)) {
			return;
		}
		...
		//rcdFilesには売上ファイルの情報(ファイル名やパス等)が格納されているため、rcdFilesからファイルの情報を取得してください。
			...
			try {
				...
				//売上ファイルから読み込んだ売上金額（salesFiles.get(2)）を既存Mapに加算するために、parseLongメソッドで型の変換を行う
				...
				Long saleAmountItem = commoditySales.get(salesFiles.get(1)) + fileSale;

				//合計金額が10桁を超えた場合（売上金額が11桁以上の場合）、エラーメッセージをコンソールに表示し、処理を終了する。
				if(saleAmount >= 10000000000L && saleAmountItem >= 10000000000L){
					System.out.println(SALEAMOUNT_OVER_TEN_DIGIT);
					return;
				}
				//加算した売上金額を既存Mapに追加
				branchSales.put(salesFiles.get(0), saleAmount);
				commoditySales.put(salesFiles.get(1), saleAmountItem);
	 ...
	 * @return 読み込み可否
	 */
	private static boolean readFile(String path, String fileName, String list, Map<String, String> mapNames, Map<String, Long> mapSales, String code) {
		BufferedReader br = null;
